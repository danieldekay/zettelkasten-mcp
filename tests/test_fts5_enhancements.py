...
def _make_note(title: str, content: str, tags: list[str] | None = None) -> Note:
    return Note(
        id=f"20240101T{next(_note_counter):015d}",
        title=title,
        content=content,
...
class TestFts5TableSchema:
    def test_fts5_table_named_fts5_notes(self, fts5_repo):
        ...
        with fts5_repo.session_factory() as session:
            row = session.execute(
                text(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='fts5_notes'"
                ),
            ).fetchone()
    ...
    def test_fts5_columns_include_en_summary_and_tags(self, fts5_repo):
        ...
        with fts5_repo.session_factory() as session:
            row = session.execute(
                text(
                    "SELECT note_id, title, content, en_summary, en_keywords, tags "
...
class TestFts5ExtendedColumnIndexing:
    def test_en_summary_and_keywords_stored_after_create(self, fts5_repo):
        ...
        with fts5_repo.session_factory() as session:
            row = session.execute(
                text(
                    "SELECT en_summary, en_keywords FROM fts5_notes WHERE note_id = :nid"
                ),
                {"nid": note.id},
            ).fetchone()
    ...
    def test_tags_searchable_in_fts5(self, fts5_repo):
        ...
        with fts5_repo.session_factory() as session:
            row = session.execute(
                text(
                    "SELECT note_id FROM fts5_notes WHERE tags MATCH 'machinelearning'"
                ),
            ).fetchone()
            assert row is not None
    ...
    def test_bm25_weights_applied(self, fts5_repo):
        ...
        body_note = _make_note(
            "Unrelated Title", "Zettelkasten mentioned in the body only."
        )
    ...
    def test_title_weight_higher_than_content(self, fts5_repo):
        ...
        filler = _make_note(
            "Filler A", "Some filler content about nothing in particular here today."
        )
        fts5_repo.create(filler)
        filler2 = _make_note(
            "Filler B", "Another unrelated note about everyday topics like cooking."
        )
        fts5_repo.create(filler2)

        title_note = _make_note(
            "zettelkastentest concept", "Ordinary content without the term."
        )
        # Give body_note a longer document so title's normalized TF wins
        body_note = _make_note(
            "Generic Title Here", f"{unique_term} " + "padding " * 20
        )
        fts5_repo.create(title_note)
        fts5_repo.create(body_note)
