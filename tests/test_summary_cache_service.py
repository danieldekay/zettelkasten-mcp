...
def db_session(tmp_path):
    """Temporary file-backed SQLite database for cache tests."""
...
class TestSummaryCacheService:
    ...
    def test_cache_miss_on_stale_hash(self, db_session):
        ...
        new_hash = "new" * 16
        svc.save_to_cache(
            db_session, "note1", old_hash, {"summary": "Old", "keywords": []}, "gpt-4"
        )
        db_session.flush()
    ...
    def test_save_overwrites_existing_entry(self, db_session):
        ...
        svc.save_to_cache(
            db_session, "note1", h, {"summary": "First", "keywords": ["x"]}, "gpt-4"
        )
        db_session.flush()
        svc.save_to_cache(
            db_session, "note1", h, {"summary": "Updated", "keywords": ["y"]}, "gpt-4"
        )
        db_session.flush()
    ...
    def test_cache_survives_across_sessions(self, tmp_path):
        ...
        with session_factory() as s1:
            ...
            svc.save_to_cache(
                s1, "note_p", h, {"summary": "Persisted", "keywords": []}, "gpt-4"
            )
            s1.commit()
