...
def calculate_content_hash(
    ...
    # JSON serialization avoids ambiguity from delimiter collisions in field data
    payload = json.dumps(
        {"title": title, "content": content, "tags": sorted_tags},
        sort_keys=True,
        separators=(",", ":"),
    )
...
class SummaryCacheService:
    ...
    def get_from_cache(
        ...
        try:
            ...
            if entry is None:
                return None
            if entry.content_hash != content_hash:  # type: ignore[misc]
                # Content has changed — cached value is stale
                logger.debug("Cache miss (hash mismatch) for note %s", note_id)
                return None

            keywords = json.loads(str(entry.en_keywords)) if entry.en_keywords else []  # type: ignore[misc]
